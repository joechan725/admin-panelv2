...
interface AdminAddressDeleteButtonProps {
  address: Address;
}

const AdminAddressDeleteButton = ({ address }: AdminAddressDeleteButtonProps) => {
  ...
  const handleDelete = async () => {
    const res = await removeAddress({ addressId: address.id, userId: param.userId });
    if (res) {
      setIsDeleting(false);
    }
  };
...
export default AdminAddressDeleteButton;
