    ...
    <div className="relative max-w-full min-h-[590px] max-h-[1080px] aspect-video flex justify-center items-center overflow-hidden">
      ...
      <div className="absolute top-1/2 right-1/2 translate-x-1/2 -translate-y-1/2 space-y-8 max-w-screen-md">
        <div className="space-y-4">
          {title && <div className="font-semibold text-4xl sm:text-5xl md:text-7xl text-center">{title}</div>}
          {content && <div className=" font-semibold text-sm sm:text-base md:text-2xl text-center">{content}</div>}
        </div>
        {link !== undefined && (
          <Link
            href={link}
            className="font-medium text-xs sm:text-base flex justify-center items-center gap-2 hover:opacity-85 underline-offset-2 hover:underline active:opacity-70"
          >
