...
const AddPasswordForm = ({ email, userInfos }: AddPasswordFormProps) => {
  ...
  const { error, isLoggingIn, isLoading, isWriting, addPassword } = useSessionStore((state) => ({
    error: state.error,
  ...
  const handleSignIn: SubmitHandler<RegisterSchema> = async (data) => {
    ...
    const res = await addPassword(password);
  ...
  return (
    <form onSubmit={handleSubmit(handleSignIn)}>
      <div className="mb-2 text-center text-lg font-semibold text-primary-text">{t('addPassword')}</div>
