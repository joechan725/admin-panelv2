...
interface searchAndOrderOrdersParameters {
  searchParams: ReadonlyURLSearchParams;
  privateOrders: Order[];
}

export const searchAndOrderOrders = ({ searchParams, privateOrders }: searchAndOrderOrdersParameters) => {
  const searchQuery = searchParams.get('search');
  const orderBy = searchParams.get('orderby');
  const itemsPerPage = +(searchParams.get('limit') ?? 10);
  const currentPage = +(searchParams.get('page') ?? 1);

  let displayOrders = privateOrders;
