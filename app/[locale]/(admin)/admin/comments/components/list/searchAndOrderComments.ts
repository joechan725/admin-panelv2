...
export const searchAndOrderComments = ({ searchParams, privateComments, locale }: searchAndOrderCommentsParameters) => {
  const searchQuery = searchParams.get('search');
  const orderBy = searchParams.get('orderby');
  const itemsPerPage = +(searchParams.get('limit') ?? 10);
  const currentPage = +(searchParams.get('page') ?? 1);

  let displayComments = privateComments;
  ...
  if (displayComments && orderBy === 'product-asc') {
    displayComments = sortObjectsByKey(displayComments, locale === 'en' ? 'productNameEN' : 'productNameZH', 'asc');
  }
  if (displayComments && orderBy === 'product-desc') {
    displayComments = sortObjectsByKey(displayComments, locale === 'en' ? 'productNameEN' : 'productNameZH', 'desc');
  }
