...
const CategoryItem = ({ privateCategory, isSelect, onSelect }: CategoryItemProps) => {
        ...
        <div className="flex items-center gap-2">
          <HoverPopup message={t('edit')}>
            <IconButton disabled={false} type="button" theme="secondary">
              <Link href={`/admin/categories/categories/${id}/edit`}>
                <Edit sizeClassName="size-5" />
              </Link>
            </IconButton>
          </HoverPopup>
          <CategoryDeleteButton privateCategory={privateCategory} />
        </div>
      </td>
    </tr>
  );
